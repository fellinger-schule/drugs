...
Das Projekt ***QTDrugPrescription*** ist eine Vorlage für die Erstellung von datenzentrierten Anwendungen. Ausgehend von dieser Vorlage  
...
4. [Verwendung der Vorlage](#verwendung-der-Vorlage)  
   ...
   2. [Abgleich mit dem QTDrugPrescription](#abgleich-mit-dem-QTDrugPrescription)  
...
## Template  
Die Struktur vom 'QTDrugPrescription' besteht aus unterschiedlichen Teilprojekten und diese in einer Gesamtlösung (im Kontext von Visual Studio ist das eine Solution) zusammengefasst. Eine Erläuterung der einzelnen Projekte, deren Typ und die Abhängigkeit finden sie in der folgenden Tabelle:  
...
|**QTDrugPrescription.Logic**|Dieses Projekt beinhaltet den vollständigen Datenzugriff, die gesamte Geschäftslogik und stellt somit den zentralen Baustein des Systems dar.|Library|CommonBase|  
|**QTDrugPrescription.WebApi**|In diesem Projekt ist die REST-Schnittstelle implementiert. Diese Modul stellt eine API (Aplication Programming Interface) für den Zugriff auf das System über das Netzwerk zur Verfügung.|Host|CommonBase, QTDrugPrescription.Logic|  
|**QTDrugPrescription.ConApp**|Dieses Projekt dient als Initial-Anwendung zum Erstellen der Datenbank, das Anlegen von Anmeldedaten falls die Authentifizierung aktiv ist und zum Importieren von bestehenden Daten. Nach der Initialisierung wird diese Anwendung kaum verwendet.|Console|CommonBase, QTDrugPrescription.Logic|  
|**QTDrugPrescription.AspMvc**|Diese Projekt beinhaltet die Basisfunktionen für eine AspWeb-Anwendung und kann als Vorlage für die Entwicklung einer einer AspWeb-Anwendung mit dem QTDrugPrescription verwendet werden.|Host|CommonBase, QTDrugPrescription.Logic|  
|**QTDrugPrescription.WpfApp**|Diese Projekt beinhaltet die Basisfunktionen für eine Wpf-Anwendung und kann als Vorlage für die Entwicklung einer einer Wpf-Anwendung mit dem QTDrugPrescription Framework verwendet werden.|Host|CommonBase, QTDrugPrescription.Logic|  
|**QTDrugPrescription.XxxYyy**|Es folgen noch weitere Vorlagen von Client-Anwendungen wie Angular, Blazor und mobile Apps. Zum jetzigen Zeitpunkt existiert nur die AspMvc-Anwendung. Die Erstellung und Beschreibung der anderen Client-Anwendungen erfolgt zu einem späteren Zeitpunkt.|Host|CommonBase, QTDrugPrescription.Logic|  
